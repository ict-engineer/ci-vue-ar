...
model-viewer{
	 <?php
	if (!empty($settings->environmentColor)){
		echo 'background-color: '. $settings->environmentColor.';' ;
	}
	?>;
}
</style>
...
?>
  ...
  <model-viewer
		  ar ar-modes="webxr scene-viewer quick-look" camera-controls alt="A 3D model of an astronaut"
		  src="../<?php echo $filename; ?>"
		  <?php

		  echo ($settings->enablePan == true) ?  'enable-pan ' : '';
		  echo ($settings->cameraControls == true) ?  'camera-controls ' : '';
		  echo ($settings->autoRotate == true) ?  'auto-rotate ' : '';
		  echo 'exposure="'.$settings->lightModel->exposure.'" ';
		  echo 'shadow-softness="'.$settings->lightModel->shadowSoftness.'" ';
		  echo 'shadow-intensity="'.$settings->lightModel->shadowIntensity.'" ';
		  echo ($settings->lightModel->useEnvironment == true) ?  'skybox-image= '.$settings->skyboxImage.' ' : '  ';
	      echo 'ar ar-modes="webxr scene-viewer quick-look" camera-controls';
...
//			}
		  ?> >
			<?php
			if ($settings->sceneLoaderSettings->progressBar){
				echo '
			...
			if ($settings->sceneLoaderSettings->arButton){
//				echo '
...
//				';
			...
			if ($settings->sceneLoaderSettings->arPrompt){
				echo '
					<div id="ar-prompt">
...
<!--		  autoplay camera-controls alt="A 3D Chair"-->
